...
class Utils_Core
{
  ...
  {
    $encryption_key = ZIPPY_WASH_PREFIX;
  ...
  {
    return array(
      'id'          => ZIPPY_WASH_PREFIX . '_divider',
      'name'       => __('', ZIPPY_WASH_PREFIX . 'woocommerce-settings-tab'),
      'type'        => 'title',
      'desc' => '<hr>'
    );
  }
  ...
  public static function get_template($template_name, $vars = array(), $absolute_path = '', $relative_path = '')
  {
    ...
    if (file_exists(ZIPPY_WASH_DIR_PATH . $relative_path . $template_name)) {
      $template = ZIPPY_WASH_DIR_PATH . $relative_path . $template_name;
    }

    $template = apply_filters_deprecated(ZIPPY_WASH_PREFIX . '\util\get_template\path_file', [$template, $vars], '1.0.0', ZIPPY_WASH_PREFIX . '\util\get_template\template');
    $template = apply_filters(ZIPPY_WASH_PREFIX . '\util\get_template\template', $template, $template_name, $absolute_path, $relative_path);
